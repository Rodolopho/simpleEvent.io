...
function attrObjectify(ele, attr,value){
		 ...
		 if(value==='value') return ele.getAttribute($impleEvent.parseJSON(attr));

		let obj={
			element:ele,
			attribute:attr,
			 value:ele.getAttribute($impleEvent.parseJSON(attr)),
			get(){
				return this.element.getAttribute($impleEvent.parseJSON(this.attribute));
			},
			set(value){
